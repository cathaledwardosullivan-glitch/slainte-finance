...
const SIMPLE_CATEGORY_MAP = {
    ...
    'Petty Cash / Other': 'Petty Cash / Other'
};
...
export default function AccountantExport({ onClose }) {
    ...
    const exportDetailedTransactions = () => {
        const csvRows = [
            ['Date', 'Description', 'Debit', 'Credit', 'Type', 'Group', 'Category', 'P&L Line', 'Comments']
        ];
    ...
    const exportAllAsZip = async () => {
        ...
        try {
            ...
            const detailedRows = [['Date', 'Description', 'Debit', 'Credit', 'Type', 'Group', 'Category', 'P&L Line', 'Comments']];
            yearTransactions.sort((a, b) => new Date(a.date) - new Date(b.date)).forEach(t => {
                const cat = t.category || {};
                detailedRows.push([
                    formatDate(t.date),
