...
const MobileLayout = () => {
    ...
    const MobileDashboard = () => (
        <div className="p-4 space-y-4 max-w-sm mx-auto">
            ...
            {/* Financial Metrics - Compact Individual Cards */}
            <div className="space-y-3">
                {/* Income */}
                <div
                    onClick={() => setSelectedMetric('income')}
                    className="bg-green-50 rounded-lg p-3 border border-green-200 cursor-pointer active:bg-green-100"
                >
                    <div className="flex justify-between items-center">
                        <div>
                            <p className="text-sm text-green-600 font-medium">Total Income</p>
                            <p className="text-xl font-bold text-green-700">{formatCurrency(summaries.income)}</p>
                        </div>
                ...
                <div
                    ...
                    className="bg-red-50 rounded-lg p-3 border border-red-200 cursor-pointer active:bg-red-100"
                >
                    <div className="flex justify-between items-center">
                        <div>
                            <p className="text-sm text-red-600 font-medium">Total Expenses</p>
                            <p className="text-xl font-bold text-red-700">{formatCurrency(summaries.expenses)}</p>
                        </div>
                ...
                <div
                    ...
                    className="bg-blue-50 rounded-lg p-3 border border-blue-200 cursor-pointer active:bg-blue-100"
                >
                    <div className="flex justify-between items-center">
                        <div>
                            <p className="text-sm text-blue-600 font-medium">Net Profit</p>
                            <p className="text-xl font-bold text-blue-700">{formatCurrency(profit)}</p>
                        </div>
                        <PieChart className="h-6 w-6 text-blue-500" />
                    </div>
                    <p className="text-xs text-blue-600 mt-1">{profitMargin}% margin • ↗ 8.2%</p>
                </div>
            </div>
        ...
        return (
            <div className="p-4 max-w-sm mx-auto">
                <div className="mb-4">
                    ...
                    <p className="text-sm text-gray-500">
                        Showing {displayedTransactions.length} of {unidentifiedTransactions.length} transactions
                    </p>
                </div>

                {/* Sort Filter */}
                {unidentifiedTransactions.length > 0 && (
                    <div className="mb-4 flex items-center justify-between bg-white rounded-lg border p-3">
                            ...
                            {displayedTransactions.map((transaction) => (
                                <div key={transaction.id} className="bg-white rounded-lg border shadow-sm">
                                    ...
                                    <div className="p-4 space-y-3">
                                            ...
                                            onChange={(e) => {
                                                if (e.target.value) {
                                                    setPendingCategorization({
                                                        ...
                                                        description: transaction.description || transaction.details || transaction.narrative || transaction.reference
                                                    });
                                                }
                                            }}
                                            defaultValue=""
                                        ...
                                        <button className="w-full text-gray-500 border border-gray-300 py-2 px-4 rounded-lg text-sm">
                                            Skip for now
                                        </button>
                                    </div>
                                </div>
                ...
                {/* AI Training Confirmation Modal */}
                {/* Step 1: Ask if they want to train AI */}
                {pendingCategorization && !showPatternSelection && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-lg w-full max-w-sm p-6">
                            <h3 className="text-lg font-bold text-gray-900 mb-3">Train AI?</h3>
                            <p className="text-sm text-gray-600 mb-4">
                                Category: <strong>{pendingCategorization.category}</strong>
                            </p>
                            <p className="text-sm text-gray-600 mb-6">
                                Should the AI learn to automatically recognize similar transactions?
                            </p>
                            <div className="space-y-3">
                                <button
                                    onClick={() => {
                                        const transaction = unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId);
                                        const defaultPattern = transaction?.description || transaction?.details || transaction?.narrative || transaction?.reference || '';
                                ...
                                <button
                                    onClick={() => {
                                        const category = categoryMapping.find(c => c.name === pendingCategorization.category);
                                        const transaction = unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId);

                                        if (category && transaction) {
                                            setTransactions(prev => [...prev, { ...transaction, category }]);
                                            setUnidentifiedTransactions(prev => prev.filter(t => t.id !== pendingCategorization.transactionId));
                                        }
                                ...
                                <button
                                    onClick={() => setPendingCategorization(null)}
                ...
                {/* Step 2: Select pattern to learn */}
                {pendingCategorization && showPatternSelection && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className="bg-white rounded-lg w-full max-w-sm p-6 max-h-[80vh] overflow-y-auto">
                            <h3 className="text-lg font-bold text-gray-900 mb-3">Choose Learning Pattern</h3>
                            <p className="text-sm text-gray-600 mb-4">
                                Select what the AI should look for in future transactions:
                            </p>

                            <div className="space-y-3 mb-6">
                                {/* Full text pattern */}
                                <label className="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-blue-50">
                                    <input
                                        type="radio"
                                        name="pattern"
                                        checked={selectedPattern === (unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.description ||
                                            unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.details || '')}
                                        onChange={() => {
                                            const transaction = unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId);
                                            setSelectedPattern(transaction?.description || transaction?.details || transaction?.narrative || transaction?.reference || '');
                                        }}
                                    ...
                                    <div className="ml-3">
                                        ...
                                        <div className="text-xs text-gray-600 mt-1">
                                            "{(unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.description ||
                                                unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.details || '').substring(0, 50)}..."
                                        </div>
                                    </div>
                                </label>
                                ...
                                {(() => {
                                    const transaction = unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId);
                                    const fullText = transaction?.description || transaction?.details || transaction?.narrative || transaction?.reference || '';
                                        ...
                                        return (
                                            <label className="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-blue-50">
                                                <input
                                                    type="radio"
                                                    name="pattern"
                                                    checked={selectedPattern === firstWord}
                                                    onChange={() => setSelectedPattern(firstWord)}
                                                    className="mt-1 flex-shrink-0"
                                                />
                                                <div className="ml-3">
                                                    <div className="font-medium text-sm">First Word</div>
                                                    <div className="text-xs text-gray-600 mt-1">
                                                        Contains "{firstWord}"
                                                    </div>
                                                </div>
                                    ...
                                    <label className="flex items-start cursor-pointer">
                                        <input
                                            ...
                                            checked={selectedPattern !== '' &&
                                                selectedPattern !== (unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.description ||
                                                    unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.details || '') &&
                                                selectedPattern !== (unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.description ||
                                                    unidentifiedTransactions.find(t => t.id === pendingCategorization.transactionId)?.details || '').split(/\s+/)[0]}
                                            onChange={() => setSelectedPattern('')}
                                            className="mt-1 flex-shrink-0"
                                        />
                                        <div className="ml-3 flex-1">
                                            <div className="font-medium text-sm mb-2">Custom Pattern</div>
                                            <input
                                                type="text"
                            ...
                            <div className="flex space-x-3">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        if (category && transaction && selectedPattern.trim()) {
                                            // Learn the pattern
                                            setLearnedIdentifiers(prev => {
                                                const newMap = new Map(prev);
                                                newMap.set(selectedPattern.trim(), category);
                                                return newMap;
                                            });
        ...
        return (
            <div className="p-4 max-w-sm mx-auto">
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500">View generated reports</p>
                </div>
                ...
                <div className="space-y-3">
                    ...
                    {savedReports.length === 0 ? (
                        <div className="bg-gray-50 border rounded-lg p-6 text-center">
                            ...
                            <p className="text-gray-600 text-sm">No saved reports</p>
                            ...
                            <div key={report.id} className="bg-white border rounded-lg p-4">
                                ...
                                <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                                    <div className="bg-green-50 p-2 rounded">
                                        ...
                                        <div className="font-bold text-green-700">
                                            {formatCurrency(report.data.totalIncome?.[report.data.year] || 0)}
                                        </div>
                                    </div>
                                    <div className="bg-blue-50 p-2 rounded">
                                        <div className="text-gray-600">Profit</div>
                                        <div className="font-bold text-blue-700">
                                            {formatCurrency(report.data.netProfit?.[report.data.year] || 0)}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex space-x-2">
                                    <button
                                        ...
                                        className="flex-1 flex items-center justify-center p-2 bg-blue-50 rounded border border-blue-200"
                                    >
                                    ...
                                    <button
                                        onClick={() => {
                                            // Email functionality - could open mail client
                                            const subject = encodeURIComponent(report.title);
                                            const body = encodeURIComponent(`Report generated on ${new Date(report.generatedDate).toLocaleDateString()}`);
                                            window.location.href = `mailto:?subject=${subject}&body=${body}`;
                                        }}
                                        className="flex-1 flex items-center justify-center p-2 bg-gray-50 rounded border border-gray-200"
                                    >
                                        <Mail className="h-4 w-4 text-gray-600 mr-1" />
                ...
                {/* Report Detail Modal - SIMPLER VERSION */}
                {selectedReport && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50">
                        <div className="bg-white rounded-t-lg sm:rounded-lg w-full sm:max-w-sm" style={{ maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
                            {/* Fixed Header */}
                            <div className="flex items-center justify-between p-4 border-b" style={{ flexShrink: 0 }}>
                                <div className="flex-1 pr-2">
                                    ...
                                    <p className="text-xs text-gray-500">
                                        {new Date(selectedReport.generatedDate).toLocaleDateString('en-IE', {
                                            day: 'numeric',
                                            month: 'short',
                                            year: 'numeric',
                                            hour: '2-digit',
                            ...
                            <div className="p-4" style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
                                {/* ... all your content here - same as before ... */}
                                <div className="space-y-4 text-sm">
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-green-50 p-3 rounded border border-green-200">
                                            <div className="text-xs text-gray-600 mb-1">Total Income</div>
                                            <div className="font-bold text-green-700">
                                                {formatCurrency(selectedReport.data.totalIncome?.[selectedReport.data.year] || 0)}
                                            </div>
                                        </div>
                                        <div className="bg-red-50 p-3 rounded border border-red-200">
                                            <div className="text-xs text-gray-600 mb-1">Total Expenses</div>
                                            <div className="font-bold text-red-700">
                                                {formatCurrency(selectedReport.data.totalExpenses?.[selectedReport.data.year] || 0)}
                                            </div>
                                        </div>
                                    </div>

                                    <div className="bg-blue-50 p-3 rounded border border-blue-200">
                                        <div className="text-xs text-gray-600 mb-1">Net Profit ({selectedReport.data.year})</div>
                                        <div className="text-lg font-bold text-blue-700">
                                            {formatCurrency(selectedReport.data.netProfit?.[selectedReport.data.year] || 0)}
                                        </div>
                                        <div className="text-xs text-gray-600 mt-1">
                                            Margin: {selectedReport.data.totalIncome?.[selectedReport.data.year] > 0
                                                ? ((selectedReport.data.netProfit?.[selectedReport.data.year] / selectedReport.data.totalIncome?.[selectedReport.data.year]) * 100).toFixed(1)
                                                : 0}%
                                        </div>
                                    </div>

                                    {selectedReport.data.expenseBreakdown && selectedReport.data.expenseBreakdown.length > 0 && (
                                        <div>
                                            <h3 className="font-semibold text-gray-900 mb-3">Expense Breakdown</h3>
                                            <div className="space-y-2">
                                                {selectedReport.data.expenseBreakdown.map((item, idx) => (
                                                    <div key={idx} className="bg-gray-50 p-3 rounded border">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2 text-xs">
                                                            <div>
                                                                <span className="text-gray-600">{selectedReport.data.year}: </span>
                                                                <span className="font-semibold">{formatCurrency(item.currentYear)}</span>
                                                            </div>
                                                            <div>
                                                                <span className="text-gray-600">{selectedReport.data.year - 1}: </span>
                                                                <span className="font-semibold">{formatCurrency(item.previousYear)}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ))}
